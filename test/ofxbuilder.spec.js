...
describe('OFX Builder: ', function() {
    ...
    it('generates checking request', function(){
        var request = ofxbuilder.buildStatementRequest(account, options);
        expect(request).toBeDefined();
        expect(request).toMatch(/^OFXHEADER:100\r\nDATA:OFXSGML\r\nVERSION:102\r\nSECURITY:NONE\r\nENCODING:USASCII\r\nCHARSET:1252\r\nCOMPRESSION:NONE\r\nOLDFILEUID:NONE\r\nNEWFILEUID:.*/);
        expect(request).toMatch(new RegExp('.+\\<DTCLIENT\\>\\d{8}.*'));
        expect(request).toMatch(new RegExp('.+\\<USERID\\>' + account.userid + '.*'));
        expect(request).toMatch(new RegExp('.+\\<USERPASS\\>' + account.userpass + '.*'));
        expect(request).toMatch(new RegExp('.+\\<ORG\\>' + account.org + '.*'));
        expect(request).toMatch(new RegExp('.+\\<FID\\>' + account.fid + '.*'));
    ...
    it('throws an error if account type is not supported',function(){
        var f = function() {
            ...
            ofxbuilder.buildStatementRequest(account, options);
        };
        expect(f).toThrow();
    });
});